...
try {
	...
	pstmt = conn.prepareStatement("select count(*)  from t_share_permissionitem t where t.sxname like ? ");
	...
	pstmt = conn.prepareStatement("SELECT * FROM (SELECT T.*,ROWNUMBER() OVER(ORDER BY t.id ASC ) AS ROWID  FROM T_SHARE_PERMISSIONITEM T where t.sxname like ? ) AS A WHERE A.ROWID > ? AND A.ROWID <= ?");
	pstmt.setString(1, "%"+keyword+"%");
	pstmt.setInt(2, pageSize * (curPage-1));
	pstmt.setInt(3, pageSize * curPage);
	rs = pstmt.executeQuery();
...
function MovePage(action) {
	...
	$("#form1").attr("action", "/hp/admin/serviceitem/serviceitemlist.jsp");
	$("#form1").submit();
}
</script>
</head>
<body>
<form action="/hp/admin/serviceitem/serviceitemlist.jsp" method="post"  name="form1" id="form1" target="categoryserviceitem">
