...
try {
	...
	pstmt = conn.prepareStatement("select count(*)  from t_share_permissionitem t where t.sxname like ? and t.id in(select sxid from tb_wsmp_category_service where categoryid = ? )");
	...
	pstmt = conn.prepareStatement("SELECT * FROM (SELECT T.*,ROWNUMBER() OVER(ORDER BY t.id ASC ) AS ROWID  FROM T_SHARE_PERMISSIONITEM T where t.sxname like ? and t.id in(select sxid from tb_wsmp_category_service where categoryid = ? )  ) AS A WHERE A.ROWID > ? AND A.ROWID <= ?");
	pstmt.setString(1, "%"+keyword+"%");
	pstmt.setString(2, category);
...
<body>
<form action="/hp/admin/serviceitem/categoryserviceitemlist.jsp" method="post"  name="form1" id="form1" >
	<input type="hidden" name="category" value="<%=category%>" />
	<input type="text" name="keyword" value="<%=keyword%>" /> <input type="submit" value="查 询"/>
